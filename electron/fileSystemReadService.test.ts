...
describe('FileSystemMediaReadService', () => {
  ...
  it('视觉自动标签仅写入 CSV 范围标签并记录丢弃统计', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'mpx-vision-auto-tags-'))
    createdRoots.push(root)

    const imagePath = path.join(root, 'vision-001.png')
    const csvPath = path.join(root, 'tags-range.csv')
    await writeTinyPng(imagePath)
    await fs.writeFile(csvPath, 'tag\ncat\ndog\n')
    ...
    const snapshot = await service.readLibrarySnapshot()
    const source = [...snapshot.image_packages, ...snapshot.image_directories].find((item) => item.images.length > 0)
    expect(source).toBeTruthy()
    if (!source) {
      throw new Error('source not found for vision auto tags')
    }
      ...
      new Response(
        JSON.stringify({
          choices: [
            {
              message: {
                content: '{"is_ad": false, "reason": "cat,dog,unknown_tag"}',
              },
            },
          ],
        }),
    ...
    try {
      const response = await service.generatePackageAutoTagsVision({
        package_id: source.id,
        tags_csv_path: csvPath,
      ...
      expect(response.generated_tags).toEqual(['cat', 'dog'])
      expect(response.analyzed_images).toBe(1)
      expect(response.dropped_tags).toEqual(['unknown_tag'])
      expect(response.invalid_response_images).toBe(0)
      ...
      expect(latestSource?.tags).toEqual(['cat', 'dog'])
      expect(eventPayloads.some((payload) => payload.reason === 'generate-package-auto-tags-vision')).toBe(true)
    } finally {
      unsubscribe()

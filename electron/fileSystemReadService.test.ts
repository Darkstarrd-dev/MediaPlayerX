...
async function createSampleAudioWithTags(audioPath: string): Promise<void> {
...
describe('FileSystemMediaReadService', () => {
  ...
  it('入库时可提取音频 ID3/Vorbis 标签并持久化到 sqlite', async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'mpx-audio-metadata-probe-'))
    createdRoots.push(root)
    ...
    const snapshot = await service.readLibrarySnapshot()
    const audio = snapshot.audios?.find((item) => path.resolve(item.absolute_path) === path.resolve(audioPath))
    ...
    service.invalidateCache()
    const refreshed = await service.readLibrarySnapshot()
    const refreshedAudio = refreshed.audios?.find((item) => item.id === audio.id)

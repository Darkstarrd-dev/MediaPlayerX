  ...
  readSnapshot(options?: { includeImages?: boolean }): LibrarySnapshotDto {
    ...
    if (includeImages) {
      ...
      for (const imageRow of imageRows) {
        const mappedRow: ImageItemDto = {
          ...
          media_locator: parseJson<MediaLocatorDto>(
            imageRow.media_locator_json,
            {
              kind: "filesystem",
              absolute_path: "",
              extension: ".jpg",
        ...
        external_metadata:
          row.source_site && row.source_url && row.source_remote_id
            ? {
                ...
                tags: parseJson<Record<string, string>>(
                  row.external_tags_json ?? "{}",
                  {},
                ),
    ...
    const videos: VideoItemDto[] = videoRows.map((row) => {
      ...
      return {
        ...
        series_id: row.series_id ?? "",
        circle:
          row.circle && row.circle.trim().length > 0 ? row.circle : "未知",
        circle_jpn: row.circle_jpn ?? "",
        author:
          row.author && row.author.trim().length > 0 ? row.author : "未知",
    ...
    const audios: AudioItemDto[] = audioRows.map((row) => {
      ...
      const trackTitle =
        row.metadata_track_title ?? row.track_title ?? defaultTrackTitle;
      return {
        id: row.id,
        file_name: row.file_name,
        absolute_path: row.absolute_path,
        ...
        track_title:
          trackTitle.trim().length > 0 ? trackTitle : defaultTrackTitle,

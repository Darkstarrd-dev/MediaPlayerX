...
const backendApi = {
  ...
  resolveMediaResource: async (request: unknown) => {
    ...
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.resolveMediaResource,
      parsed,
    );
  ...
  readClipboardImportPaths: async () => {
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.readClipboardImportPaths,
    );
  ...
  readAudioEngineState: async () => {
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.readAudioEngineState,
    );
    return readAudioEngineStateResponseSchema.parse(response);
  },
  readAudioEnginePlaybackStatus: async () => {
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.readAudioEnginePlaybackStatus,
    );
    return readAudioEnginePlaybackStatusResponseSchema.parse(response);
  },
  readAudioEngineAnalysisFrame: async () => {
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.readAudioEngineAnalysisFrame,
    );
    return readAudioEngineAnalysisFrameResponseSchema.parse(response);
  },
  ...
  verifyAudioTranscodeFfmpegBin: async (request: unknown) => {
    const parsed = verifyAudioTranscodeFfmpegBinRequestSchema.parse(request);
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.verifyAudioTranscodeFfmpegBin,
      parsed,
    );
    return verifyAudioTranscodeFfmpegBinResponseSchema.parse(response);
  },
  listAudioOutputDevices: async () => {
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.listAudioOutputDevices,
    );
  ...
  audioEngineStopPlayback: async () => {
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.audioEngineStopPlayback,
    );
    return audioEngineActionResponseSchema.parse(response);
  },
  ...
  flushSubtitleSession: async () => {
    const response = await ipcRenderer.invoke(
      BACKEND_CHANNELS.flushSubtitleSession,
    );
...
const platformApi = {
  getPathForFile: (file: File): string | null => {
    ...
    return typeof fallback.path === "string" && fallback.path.trim().length > 0
      ? fallback.path
      : null;
  },
};
...
const windowApi = {
  ...
  setFullscreen: async (active: boolean) => {
    await ipcRenderer.invoke(
      APP_WINDOW_CHANNELS.setFullscreen,
      Boolean(active),
    );
  },
  isMaximized: async () => {
  ...
  writeClipboardPng: async (pngBytes: Uint8Array) => {
    return await ipcRenderer.invoke(
      APP_WINDOW_CHANNELS.writeClipboardPng,
      pngBytes,
    );
  },
  ...
  setNativeChromeEnabled: async (enabled: boolean) => {
    return await ipcRenderer.invoke(
      APP_WINDOW_CHANNELS.setNativeChromeEnabled,
      Boolean(enabled),
    );
  },

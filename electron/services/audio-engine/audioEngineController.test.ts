...
describe("AudioEngineController mpv restart guard", () => {
  beforeEach(() => {
    ...
    mockState.readPlaybackStatus.mockClear();
    mockState.readAnalysisFrame.mockClear();
    mockState.onProcessExit = null;
  });

  it("mpv 异常退出后应自动拉起并保持增强模式", async () => {
    const controller = new AudioEngineController({
      projectRoot: "C:/opencode/MediaPlayer",
    });
  ...
  it("读取状态与分析帧应启用短周期缓存，避免高频拉取", async () => {
    const controller = new AudioEngineController({
      projectRoot: "C:/opencode/MediaPlayer",
    });
    ...
    await controller.readPlaybackStatus();
    await controller.readPlaybackStatus();
    expect(mockState.readPlaybackStatus).toHaveBeenCalledTimes(1);

    await controller.readAnalysisFrame();
    await controller.readAnalysisFrame();
    expect(mockState.readAnalysisFrame).toHaveBeenCalledTimes(1);
  });
});

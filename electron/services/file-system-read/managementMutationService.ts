...
const IMAGE_CONVERT_TARGET_EXTENSION_BY_FORMAT: Record<StartImageConvertTaskRequestDto['target_format'], string> = {
  webp: '.webp',
  jpeg: '.jpg',
  png: '.png',
  avif: '.avif',
...
interface ManagementMutationServiceOptions {
  ...
  refreshSnapshotFromFilesystem?: (options?: { force?: boolean }) => Promise<LibrarySnapshotDto>
...
export class ManagementMutationService {
  ...
  private throwIfImageConvertCancelled(isCancelled: (() => boolean) | undefined): void {
    if (isCancelled?.()) {
      throw new ImageConvertCancelledError()
    }
  }

  private buildImageConvertTempPath(targetPath: string, suffix: string): string {
    return `${targetPath}.${Date.now()}.${Math.round(Math.random() * 100_000)}.${suffix}`
  }

  private async replaceFileContentAtomically(sourcePath: string, targetPath: string, content: Buffer): Promise<void> {
    ...
    const tempPath = this.buildImageConvertTempPath(resolvedTargetPath, 'mpx-tmp')

    await fs.mkdir(path.dirname(resolvedTargetPath), { recursive: true })
    await fs.writeFile(tempPath, content)

    if (sourceKey === targetKey) {
      const backupPath = this.buildImageConvertTempPath(resolvedSourcePath, 'mpx-bak')
      ...
      try {
        ...
        if (!replaced) {
          await fs.rename(backupPath, resolvedSourcePath).catch(() => undefined)
        }
  ...
  private async convertImageBuffer(
    sourceBuffer: Buffer,
    request: Pick<StartImageConvertTaskRequestDto, 'scale_factor' | 'longest_edge_px' | 'target_format' | 'quality'>,
  ): Promise<Buffer> {
    ...
    if (shouldResizeByScale || shouldResizeByLongestEdge) {
      ...
      if (Number.isFinite(width) && Number.isFinite(height) && width > 0 && height > 0) {
        ...
        if (shouldResizeByLongestEdge) {
          ...
          const targetLongestEdge = Math.max(1, Math.round(request.longest_edge_px ?? sourceLongestEdge))
    ...
    if (request.target_format === 'png') {
      return await pipeline.png({ quality: request.quality, compressionLevel: 9 }).toBuffer()
    }
    return await pipeline.avif({ quality: request.quality }).toBuffer()
  }

  private toConvertedEntryName(entryName: string, targetExtension: string): string {
  ...
  private countConvertibleItemsFromSource(source: ImagePackageDto): number {
    const hasArchiveEntries = source.images.some((image) => image.media_locator.kind === 'archive-entry')
    if (hasArchiveEntries) {
      return source.images.filter((image) => image.media_locator.kind === 'archive-entry').length
    }
    return source.images.filter((image) => image.media_locator.kind === 'filesystem').length
  }

  private collectImageConvertSources(
    nodeIds: string[],
    snapshot: LibrarySnapshotDto,
  ): { selectedSources: ImagePackageDto[]; failedNodeIds: Array<{ node_id: string; reason: string }> } {
    ...
    for (const nodeId of nodeIds) {
      ...
      if (parsed.kind === 'video' || parsed.kind === 'audio') {
        failedNodeIds.push({
          node_id: nodeId,
          reason: 'unsupported node kind',
        })
        continue
      }
  ...
  private async convertDirectorySource(
    ...
    updateProgress: (payload: { processedDelta: number; successDelta: number; failedDelta: number; message: string }) => void,
  ): Promise<{ firstError: string | null }> {
    const targetExtension = IMAGE_CONVERT_TARGET_EXTENSION_BY_FORMAT[request.target_format]
    const filePaths = Array.from(
      new Set(
        source.images
          .map((image) => image.media_locator)
          .filter((locator): locator is Extract<ImagePackageDto['images'][number]['media_locator'], { kind: 'filesystem' }> => locator.kind === 'filesystem')
    ...
    const workerCount = Math.max(1, Math.min(request.concurrency, queue.length))
    await Promise.all(
      Array.from({ length: workerCount }, async () => {
        while (queue.length > 0) {
          ...
          try {
            ...
            const convertedBuffer = await this.convertImageBuffer(sourceBuffer, request)
            const nextPath = path.resolve(path.join(path.dirname(sourcePath), `${path.basename(sourcePath, path.extname(sourcePath))}${targetExtension}`))
            await this.replaceFileContentAtomically(sourcePath, nextPath, convertedBuffer)
            updateProgress({
              processedDelta: 1,
              successDelta: 1,
              failedDelta: 0,
  ...
  private async convertZipSource(
    ...
    const targetExtension = IMAGE_CONVERT_TARGET_EXTENSION_BY_FORMAT[request.target_format]
    const sourceImageCount = source.images.filter((image) => image.media_locator.kind === 'archive-entry').length
      ...
      await this.withArchiveWriteLock(archivePath, async () => {
        ...
        for (const entry of entries) {
          ...
          if (plannedEntryNameSet.has(nextEntryName)) {
            throw new Error(`archive entry destination already exists: ${nextEntryName}`)
          }
        ...
        const tempPath = this.buildImageConvertTempPath(archivePath, 'mpx-tmp.zip')
        const backupPath = this.buildImageConvertTempPath(archivePath, 'mpx-bak')
  ...
  async runImageConvertTask(
    ...
    const collected = this.collectImageConvertSources(normalizedNodeIds, snapshot)
    ...
    if (selectedSources.length <= 0) {
      throw new Error('image convert failed: selected sources are outside allowlist')
    }

    const blockedCount = collected.selectedSources.length - selectedSources.length
    const blockedItemCount = collected.selectedSources
      .filter((source) => !selectedSources.some((item) => item.id === source.id))
      .reduce((total, source) => total + this.countConvertibleItemsFromSource(source), 0)
    const selectionErrorCount = collected.failedNodeIds.length

    const totalCount = selectedSources.reduce((total, source) => total + this.countConvertibleItemsFromSource(source), 0)
    if (totalCount <= 0) {
      throw new Error('image convert failed: selected source has no convertible image')
    }
    ...
    for (const source of selectedSources) {
      ...
      const updateProgress = (payload: { processedDelta: number; successDelta: number; failedDelta: number; message: string }): void => {
      ...
      const shouldUseZipFlow =
        ...
        source.images.some((image) => image.media_locator.kind === 'archive-entry')
      const next = shouldUseZipFlow
        ? await this.convertZipSource(source, request, options, updateProgress)
        : await this.convertDirectorySource(source, request, options, updateProgress)
    ...
    if (successCount > 0) {
      ...
      if (changedArchivePaths.size > 0) {
        await this.options.refreshArchiveIndexesForPaths(changedArchivePaths).catch(() => undefined)
      }

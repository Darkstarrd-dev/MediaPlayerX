...
function createSnapshotForVideos(videoPaths: string[]): LibrarySnapshotDto {
  return {
    image_packages: [],
    image_directories: [],
    videos: videoPaths.map((absolutePath, index) =>
      createVideoFixture(`video-${index + 1}`, absolutePath),
    ),
    audios: [],
  };
}
  ...
  const service = new ManagementMutationService({
    ...
    ensureRuntimeDependencies: vi.fn().mockResolvedValue({
      ...
      subtitleEngine: {
        installed: false,
        loadable: false,
        optionalComponentInstalled: false,
        source: "none",
        moduleRoot: null,
        optionalComponentRoot: null,
        providers: {
          cpu: false,
          directml: false,
        },
...
describe("ManagementMutationService.runAudioTranscodeTask import source sync", () => {
  ...
  it("转码成功后应将输出目录写入 import_sources 并刷新快照", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-audio-transcode-import-sync-"),
    );
    tempRoots.push(rootDir);

    const importPathRegistry = new ImportPathRegistry();
    let importSourcesState = {
      directories: [path.join(rootDir, "existing")],
      files: [path.join(rootDir, "existing-file.flac")],
    };
    importPathRegistry.hydrate(importSourcesState);

    const database = {
      readImportSources: vi.fn(() => importSourcesState),
      writeImportSources: vi.fn(
      ...
      readMusicImportSources: vi.fn(() => ({ directories: [], files: [] })),
    ...
    const service = new ManagementMutationService({
      ...
      refreshArchiveIndexesForPaths: vi.fn().mockResolvedValue(undefined),
      pruneArchiveIndexesByDeletedRoots: vi.fn(),
      removeImportSourcePaths: vi.fn().mockResolvedValue(undefined),
      replaceImportSourcePaths: vi.fn().mockResolvedValue(undefined),
      buildMediaAccessContext: () => buildAccessContext(rootDir),
      emitLibraryChanged,
    });

    const audioTranscodeService = (
      service as unknown as {
        audioTranscodeService: {
          runAudioTranscodeTask: (
            request: unknown,
            options: unknown,
          ) => Promise<{
            total_count: number;
    ...
    vi.spyOn(audioTranscodeService, "runAudioTranscodeTask").mockResolvedValue({
      total_count: 1,
      processed_count: 1,
      success_count: 1,
      failed_count: 0,
    ...
    expect(database.writeImportSources).toHaveBeenCalledTimes(1);
    expect(importSourcesState.directories).toContain(
      path.join(rootDir, "transcoded", "audio"),
    );
    expect(importPathRegistry.getImportDirectoryRoots()).toContain(
      path.join(rootDir, "transcoded", "audio"),
    );
    expect(refreshSnapshotFromFilesystem).toHaveBeenCalledTimes(1);
    expect(emitLibraryChanged).toHaveBeenCalledWith({
...
describe("ManagementMutationService.runVideoTranscodeTask import source sync", () => {
  const tempRoots: string[] = [];
  ...
  it("视频转码成功后应将输出目录写入 import_sources 并刷新快照", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-video-transcode-import-sync-"),
    );
    tempRoots.push(rootDir);
    ...
    let importSourcesState = {
      directories: [path.join(rootDir, "existing")],
      files: [path.join(rootDir, "existing-file.mp4")],
    };
    importPathRegistry.hydrate(importSourcesState);
    ...
    vi.spyOn(videoTranscodeService, "runVideoTranscodeTask").mockResolvedValue({
      total_count: 1,
      processed_count: 1,
      success_count: 1,
      failed_count: 0,
    ...
    expect(database.writeImportSources).toHaveBeenCalledTimes(1);
    expect(importSourcesState.directories).toContain(
      path.join(rootDir, "transcoded", "video"),
    );
    expect(importPathRegistry.getImportDirectoryRoots()).toContain(
      path.join(rootDir, "transcoded", "video"),
    );
...
describe("ManagementMutationService.moveSidebarNodes", () => {
  ...
  it("分组目录已存在时应直接报错并且不触发后续写入", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-move-group-exists-"),
    );
    tempRoots.push(rootDir);
    ...
    await fs.mkdir(path.join(destinationRootDir, "group-a"), {
      recursive: true,
    });
    ...
    await expect(
      ...
      `分组失败：目录已存在 ${path.resolve(destinationRootDir, "group-a")}`,
    );
  ...
  it("rename 返回 EXDEV 时应回退 copy+remove 并完成快照更新", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-move-exdev-"),
    );
    tempRoots.push(rootDir);
    ...
    const targetPath = path.resolve(
      path.join(destinationRootDir, "pkg-exdev.zip"),
    );
    ...
    vi.spyOn(fs, "rename").mockImplementation(async (fromPath, toPath) => {
      if (
        path.resolve(String(fromPath)) === sourceResolved &&
        path.resolve(String(toPath)) === targetPath
      ) {
    ...
    await expect(fs.stat(targetPath)).resolves.toMatchObject({
    ...
    expect(harness.pruneArchiveIndexesByDeletedRoots).toHaveBeenCalledWith([
      sourceResolved,
    ]);
    expect(harness.refreshArchiveIndexesForPaths).toHaveBeenCalledWith([
      targetPath,
    ]);
  ...
  it("部分节点冲突时应返回部分成功并保留失败明细", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-move-partial-"),
    );
    tempRoots.push(rootDir);
    ...
    await expect(fs.stat(sourcePathA)).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(fs.stat(movedPathA)).resolves.toMatchObject({
      isFile: expect.any(Function),
    });
    await expect(fs.stat(sourcePathB)).resolves.toMatchObject({
      isFile: expect.any(Function),
    });
...
describe("ManagementMutationService.renameSidebarNode", () => {
  ...
  it("重命名视频文件时未输入扩展名应保留原扩展名", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-video-ext-"),
    );
    tempRoots.push(rootDir);
  ...
  it("重命名视频文件时允许名称包含点号并自动保留原扩展名", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-video-change-ext-"),
    );
    tempRoots.push(rootDir);
    ...
    const sourceResolved = path.resolve(sourcePath);
    const targetPath = path.resolve(
      path.join(rootDir, "videos", "clip-b.mkv.mp4"),
    );
    const expectedMappings = [{ fromPath: sourceResolved, toPath: targetPath }];
...
describe("ManagementMutationService.renameItems", () => {
  ...
  it("应支持 image-item 的 preview-only 与 metadata 模板预览", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-items-preview-"),
    );
    tempRoots.push(rootDir);
    ...
    expect(response.results[0].target_name).toBe(
      "AuthorJp(AuthorEn)-CircleJp(CircleEn) - TitleJp.jpg",
    );
    expect(harness.database.moveSnapshotEntriesByPaths).not.toHaveBeenCalled();
  ...
  it("replace 模式应可删除完整长片段（含符号与空格）", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-items-replace-long-"),
    );
    tempRoots.push(rootDir);
    ...
    const sourcePath = path.join(
      rootDir,
      "videos",
      "夏の初撮り - H動漫-裏番-線上看 - Hanime1.me.mp4",
    );
    await ensureFile(sourcePath);
    ...
    const response = await harness.service.renameItems({
      targets: [
        {
          kind: "sidebar-node",
          node_id: "video:夏の初撮り - H動漫-裏番-線上看 - Hanime1.me.mp4",
        },
      ],
      mode: "replace",
  ...
  it("replace 模式未命中时应返回 replace-target-not-found", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-items-replace-no-match-"),
    );
    tempRoots.push(rootDir);
  ...
  it("应支持 archive-entry 改名并回写 zip 与数据库映射", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-items-zip-"),
    );
    tempRoots.push(rootDir);
  ...
  it("fail_fast 开启时命中重名冲突应中断并不落盘", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-items-failfast-"),
    );
    tempRoots.push(rootDir);
  ...
  it("预览保留重名冲突错误，执行时自动为重复目标追加数字后缀", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-items-dup-fix-"),
    );
    tempRoots.push(rootDir);
    ...
    expect(
      previewResponse.failed.some((item) =>
        item.reason.includes("duplicate destination"),
      ),
    ).toBe(true);
    ...
    const targetNames = executeResponse.results
  ...
  it("archive-entry 非图片扩展名应拒绝重命名", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-manage-rename-items-non-image-"),
    );
    tempRoots.push(rootDir);
...
describe("ManagementMutationService.runImageConvertTask", () => {
  ...
  it("目录目标重名冲突时应失败并保持源文件与目标文件不变", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-image-convert-dir-collision-"),
    );
    tempRoots.push(rootDir);
  ...
  it("zip 原子替换失败时应回滚到原始压缩包", async () => {
    const rootDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "mpx-image-convert-zip-rollback-"),
    );
    tempRoots.push(rootDir);
    ...
    await writeStoredZipFromEntries(archivePath, [
      {
        entryName: "01.png",
        content: Buffer.from(ONE_PIXEL_PNG_BASE64, "base64"),
      },
      {
        entryName: "02.png",
        content: Buffer.from(ONE_PIXEL_PNG_BASE64, "base64"),

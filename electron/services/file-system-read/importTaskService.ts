...
interface ImportTaskServiceOptions {
  ...
  emitLibraryChanged: (payload: {
    reason: "import-task-updated" | "import-task-finished";
...
export class ImportTaskService {
  ...
  recoverInterruptedImportTasks(): void {
    ...
    for (const task of tasks) {
      ...
      if (
        task.status === "running" &&
        this.runningImportTaskIds.has(task.taskId)
      ) {
        continue;
      }
      ...
      this.options.database.upsertTask({
        ...
        status: "failed",
        progress:
          task.totalCount > 0 ? task.processedCount / task.totalCount : 1,
        message:
          task.status === "running"
            ? "导入任务已中断，请重试"
            : "导入任务未执行，请重试",
  ...
  async enqueueImportTask(
    ...
    const normalizedPaths = Array.from(
      new Set(request.paths.map((value) => value.trim()).filter(Boolean)),
    );
  ...
  async readImportTasks(): Promise<{ tasks: ImportTaskDto[] }> {
    const tasks = this.options.database
      .readTasks()
      .map((record) => this.toImportTaskDto(record));
  ...
  private async runImportTask(taskId: string): Promise<ImportTaskDto> {
    ...
    const musicImportMode = task
      ? this.isMusicImportTaskSource(task.taskSource)
      : false;
    ...
    const finalTask = await executeImportTask({
      ...
      handleImportStageKnownHashHits:
        this.options.handleImportStageKnownHashHits,
    });

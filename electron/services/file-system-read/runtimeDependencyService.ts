...
import {
  probeSubtitleEngineStatus,
  type SubtitleEngineStatusSnapshot,
} from "../../subtitles/subtitleEngineProbe";
...
export class RuntimeDependencyService {
  ...
  private runtimeDependencyLoadingPromise: Promise<RuntimeDependencySnapshot> | null =
    null;
  ...
  overrideBinaryPaths(nextFfmpegBin: string, nextFfprobeBin: string): void {
    this.ffmpegBin = nextFfmpegBin;
    this.ffprobeBin = nextFfprobeBin;
    this.runtimeDependencySnapshot = null;
    this.runtimeDependencyLoadingPromise = null;
  }

  private async loadRuntimeDependencies(): Promise<RuntimeDependencySnapshot> {
    const [sharpModule, ffmpeg, ffprobe, archiveWasm, powershell] =
      await Promise.all([
        getSharpModule(),
        checkCommandAvailability(this.ffmpegBin, ["-version"]),
        checkCommandAvailability(this.ffprobeBin, ["-version"]),
        readArchiveWasmSupport(),
        checkCommandAvailability("powershell.exe", [
          "-NoProfile",
          "-Command",
    ...
    return {
      ...
      ffprobe,
      sevenZip:
        Boolean(sharpModule?.default) &&
        archiveWasm.rar &&
        archiveWasm.sevenZip,
      powershell,
  ...
  async ensureRuntimeDependencies(): Promise<RuntimeDependencySnapshot> {
    ...
    if (!this.runtimeDependencyLoadingPromise) {
      this.runtimeDependencyLoadingPromise =
        this.loadRuntimeDependencies().finally(() => {
          this.runtimeDependencyLoadingPromise = null;
        });
    }
  ...
  async readRuntimeCapabilities(): Promise<ReadRuntimeCapabilitiesResponseDto> {
    ...
    const subtitleDirectMlStatus = subtitleEngine.installed
      ? "available"
      : "unavailable";
    ...
    return readRuntimeCapabilitiesResponseSchema.parse({
      ...
      minimum_matrix: [
        ...
        },
        {
          ...
          note: dependencies.sharp
            ? "Sharp 可用，启用 thumbnail 变体缓存"
            : "Sharp 缺失，自动回退 original 变体",
        },
        {
          capability: "视频元数据探测（ffprobe）",
          status: dependencies.ffprobe ? "available" : "degraded",
          note: dependencies.ffprobe
            ? "ffprobe 可用，读取真实时长与分辨率"
            : "ffprobe 缺失，使用默认时长与分辨率",
        },
        {
          capability: "视频封面抓取（ffmpeg）",
          status: dependencies.ffmpeg ? "available" : "degraded",
          note: dependencies.ffmpeg
            ? "ffmpeg 可用，支持 Save as cover 真实截帧"
            : "ffmpeg 缺失，仅保留封面颜色写入",
        },
        {
          capability: "rar/7z 归一化",
          status: dependencies.sevenZip ? "available" : "unavailable",
          note: dependencies.sevenZip
            ? "WASM 解包器 + Sharp 可用，归一化为 zip(store)"
            : "WASM 解包器或 Sharp 不可用，rar/7z 图包被跳过并记录告警",
        },
        {

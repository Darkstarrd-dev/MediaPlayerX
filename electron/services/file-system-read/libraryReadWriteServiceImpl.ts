...
function filterHiddenImagesFromSource(
...
function filterHiddenImagesFromSources(
...
export class LibraryReadWriteService {
  ...
  private async executeSubtitleCleanupStage(
    ...
    try {
      ...
      await this.runCleanupLlmStreaming(
        taskId,
        llmEndpoint,
        llmModel,
        task.raw_subtitle_text,
        llmPrompt,
      );
  ...
  private readSubtitleAsrSettings(): {
    modelDir: string;
    modelId: string;
    language: string;
  } {
    const rawState = this.options.database.readAppState<unknown>(
      SETTINGS_STATE_KEY,
      null,
    );
    const settings = (
  ...
  private async transcribeVideoToSrt(videoPath: string): Promise<string> {
    ...
    const cacheAudioPath = path.join(
      cacheDir,
      `${makeStableId("subtitle-cleanup-audio", videoPath)}.f32le`,
    );

...
export class ManageAdReviewService {
  ...
  async startManageAdReview(
    ...
    const initialStatus: ManageAdReviewTaskDto['status'] = noNeedToRun ? 'review' : hasRunningTask ? 'pending' : 'running'
    const task: ManageAdReviewTaskDto = {
      ...
      message: noNeedToRun ? '已审核(未变更)，无需执行' : initialStatus === 'pending' ? '排队中，等待前序任务完成' : '广告审核任务进行中',
      error_detail: null,
      candidates: [],
      created_at_ms: now,
      updated_at_ms: now,
  ...
  async confirmManageAdReviewDelete(
    ...
    const nextCandidates = queueItem.task.candidates.filter((item) => !deletedImageIdSet.has(item.image_id))

    const nextTask: ManageAdReviewTaskDto = {
      ...queueItem.task,
      candidates: nextCandidates,
      suspected_count: nextCandidates.length,
      scope_image_ids: queueItem.task.scope_image_ids.filter((imageId) => !deletedImageIdSet.has(imageId)),
  ...
  private normalizeStoredRequest(
    rawRequest: unknown,
    task: ManageAdReviewTaskDto,
  ): StartManageAdReviewRequestDto | null {
  ...
  private readQueueStateInternal(): PersistedQueueState {
    ...
    for (const rawItem of rawItems) {
      ...
      const taskResult = manageAdReviewTaskSchema.safeParse((rawItem as { task?: unknown }).task)
  ...
  private updateQueueTask(
    taskId: string,
    updater: (task: ManageAdReviewTaskDto, item: PersistedQueueItem) => ManageAdReviewTaskDto,

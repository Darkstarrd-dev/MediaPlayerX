...
import type {
  ImageEntryRef,
  ManageCoverReviewServiceOptions,
  PersistedQueueItem,
  PersistedQueueState,
...
export class ManageCoverReviewService {
  ...
  async startManageCoverReview(
    ...
    const initialStatus: ManageCoverReviewTaskDto["status"] = noNeedToRun
      ? "review"
      : hasRunningTask
        ? "pending"
        : "running";
  ...
  async confirmManageCoverReviewHide(
    ...
    if (!queueItem) {
      throw new Error(
        `封面封底审核应用隐藏失败：任务不存在 ${request.task_id}`,
      );
    }
    ...
    const appliedCount = Math.min(
      requestedCount,
      Math.max(0, response.updated_count),
    );
    ...
    const nextCandidates = queueItem.task.candidates.filter(
      (item) => !hiddenImageIdSet.has(item.image_id),
    );
    ...
    const nextTask: ManageCoverReviewTaskDto = {
      ...
      scope_image_ids: queueItem.task.scope_image_ids.filter(
        (imageId) => !hiddenImageIdSet.has(imageId),
      ),
          ...
          },
          {
            ...
            onEvent: (event) => {
              ...
              const nextImageSourceById: Record<
                string,
                ManageCoverReviewImageSourceDto
              > = {
              ...
              if (event.status === "suspected" && imageEntry) {
                const normalizedReason =
                  event.reason.trim() || "suspected_non_body";
  ...
  private updateQueueTask(
    ...
    queue.items = queue.items.map((item) => {
      ...
      const nextTask = manageCoverReviewTaskSchema.parse(
        updater(item.task, item),
      );
      changed = true;
      updatedItem = {
        ...item,

...
export const appSettingsSchema = z.object({
  ...
  wdSwinTaggerAutoTagGeneralMinScore: z.number().min(0).max(1),
  wdSwinTaggerAutoTagCharacterMinScore: z.number().min(0).max(1),
  wdSwinTaggerAutoTagIncludeRating: z.boolean(),
  wdSwinTaggerAutoTagRatingMinScore: z.number().min(0).max(1),
  visionAutoTagCsvPath: z.string().max(1024),
  visionAutoTagSampleImageCount: z.number().int().min(1).max(24),
  visionAutoTagOccurrenceThreshold: z.number().int().min(1).max(24),
  visionAutoTagTemperature: z.number().min(0).max(1),
  visionAutoTagTimeoutMs: z.number().int().min(3_000).max(120_000),

...
function isResolvedMediaApplyMode(
  value: string,
): value is ResolvedMediaApplyMode {
  return value === "raf" || value === "immediate";
}

function isResolvedMediaStateScope(
  value: string,
): value is ResolvedMediaStateScope {
...
function parseResolvedMediaTuning(
  config: Record<string, unknown>,
): ResolvedMediaTuning {
  const raw = isRecord(config.resolvedMedia) ? config.resolvedMedia : {};
  const applyModeRaw =
    typeof raw.applyMode === "string"
      ? raw.applyMode
      : typeof raw.apply_mode === "string"
        ? raw.apply_mode
        : "";
  const stateScopeRaw =
    typeof raw.stateScope === "string"
      ? raw.stateScope
      : typeof raw.state_scope === "string"
        ? raw.state_scope
        : "";
  ...
  return {
    applyMode: isResolvedMediaApplyMode(applyModeRaw)
      ? applyModeRaw
      : undefined,
    stateScope: isResolvedMediaStateScope(stateScopeRaw)
      ? stateScopeRaw
      : undefined,
    maxConcurrent:
...
function parseSkeletonTuning(
  config: Record<string, unknown>,
): ImageLoadingSkeletonTuning {
  const raw = isRecord(config.imageLoadingSkeleton)
    ? config.imageLoadingSkeleton
    : {};
...
function parseE2eTuning(config: Record<string, unknown>): UiBenchE2eTuning {
  ...
  const importPaths = importPathsRaw
    ? importPathsRaw.filter(
        (value): value is string =>
          typeof value === "string" && value.trim().length > 0,
      )
    : undefined;

  const browseStepsRaw =
    typeof raw.browseSteps === "number"
      ? raw.browseSteps
      : typeof raw.browse_steps === "number"
        ? raw.browse_steps
        : null;
  const browseIntervalRaw =
    typeof raw.browseIntervalMs === "number"
      ...
      : typeof raw.browse_interval_ms === "number"
        ...
        : null;
  const warmupRaw =
    typeof raw.warmupMs === "number"
      ? raw.warmupMs
      : typeof raw.warmup_ms === "number"
        ? raw.warmup_ms
        : null;
  ...
  const waitImportCompletionRaw =
    parseOptionalBoolean(
      raw.waitImportCompletion ?? raw.wait_import_completion,
    ) ??
    parseOptionalBoolean(
      config.waitImportCompletion ?? config.wait_import_completion,
    );

  return {
    importPaths,
    browseSteps:
      typeof browseStepsRaw === "number" &&
      Number.isFinite(browseStepsRaw) &&
      browseStepsRaw >= 0
        ? Math.round(browseStepsRaw)
        : undefined,
    browseIntervalMs:
      typeof browseIntervalRaw === "number" &&
      Number.isFinite(browseIntervalRaw) &&
      browseIntervalRaw > 0
        ? Math.round(browseIntervalRaw)
        : undefined,
    warmupMs:
      typeof warmupRaw === "number" &&
      Number.isFinite(warmupRaw) &&
      warmupRaw >= 0
        ? Math.round(warmupRaw)
        : undefined,
    maxDurationMs:
      typeof maxDurationRaw === "number" &&
      Number.isFinite(maxDurationRaw) &&
      maxDurationRaw > 0
        ? Math.round(maxDurationRaw)
        : undefined,
...
function BenchRoot({ benchMode }: BenchRootProps) {
  ...
  const [error, setError] = useState<string | null>(
    mode ? null : `unsupported bench mode: ${benchMode}`,
  );
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      const response = await window.mediaPlayerBench
        ?.readConfig()
        .catch(() => null);
      ...
      const candidateId =
        typeof config.candidateId === "string"
          ? config.candidateId
          : typeof config.candidate_id === "string"
            ? config.candidate_id
            : "candidate";
      const runTag =
        typeof config.runTag === "string"
          ? config.runTag
          : typeof config.run_tag === "string"
            ? config.run_tag
            : buildRunTag();
      ...
      setBenchSettings({
        ...
        candidateId,
        runTag,
        librarySnapshotLite: parseOptionalBoolean(
          config.librarySnapshotLite ?? config.library_snapshot_lite,
        ),
        importRefreshThrottle: parseOptionalBoolean(
          config.importRefreshThrottle ?? config.import_refresh_throttle,
        ),

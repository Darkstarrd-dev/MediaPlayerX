...
interface SidebarRenameDialogProps {
  ...
  onModeChange: (
    value: "single" | "replace" | "numbering" | "remove-range" | "metadata",
  ) => void;
...
function SidebarRenameDialog({
  ...
  const shouldVirtualizePreviewRows =
    batchModeActive &&
    previewRows.length > previewVirtualizeThreshold &&
    previewListViewportHeight > 0;
  ...
  const previewVirtualRange = useMemo(() => {
    ...
    const unclampedStartIndex = Math.max(
      0,
      Math.floor(safeScrollTop / previewRowHeight) - overscanRows,
    );
    const startIndex = Math.min(
      Math.max(0, previewRows.length - 1),
      unclampedStartIndex,
    );
    const visibleCount =
      Math.ceil(previewListViewportHeight / previewRowHeight) +
      overscanRows * 2;
    ...
    previewListViewportHeight,
    previewRows.length,
    shouldVirtualizePreviewRows,
  ]);

  const previewRowsForRender = useMemo(
    () =>
      previewRows.slice(
        previewVirtualRange.startIndex,
        previewVirtualRange.endIndex,
      ),
  ...
  if (!batchModeActive) {
    return (
      <div
        ...
        data-slot="fg-sidebar-shortcut-rename-single-ovl"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        >
          <h3>{inputLabel}</h3>
          {errorMessage ? (
            <p className="sidebar-rename-error">{errorMessage}</p>
          ) : null}
          <input
  ...
  return (
    <div
      ...
      data-slot="fg-sidebar-shortcut-rename-ovl"
      role="dialog"
      aria-modal="true"
      aria-label={inputLabel}
    ...
    >
      ...
      >
        <header
          className="mpx-large-panel-head settings-head settings-head-draggable sidebar-rename-header"
          {...headHandlers}
        >
          <div className="sidebar-rename-header-main">
            <h3 className="sidebar-rename-title">{inputLabel}</h3>
            {previewSummaryText ? (
              <p className="sidebar-rename-preview-summary">
                {previewSummaryText}
              </p>
            ) : null}
          </div>
          {errorMessage ? (
            <p className="sidebar-rename-error sidebar-rename-error-header">
              {errorMessage}
            </p>
          ) : null}
          <button
        ...
        <div className="mpx-large-panel-shell settings-shell is-no-side sidebar-rename-shell">
          <main className="mpx-large-panel-main settings-main sidebar-rename-main">
            {batchModeActive ? (
              <>
                <div className="sidebar-rename-merge-shell mpx-overlay-merged-stack">
                  {mode === "replace" ? (
                    <div
                      className="sidebar-rename-replace-controls mpx-overlay-merged-top mpx-overlay-seamless-row"
                      ...
                      <label className="sidebar-rename-seamless-control sidebar-rename-mode-control">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            onModeChange(
                              event.target
                                .value as SidebarRenameDialogProps["mode"],
                            )
                          }
                        >
                          ...
                          <option value="remove-range">
                            {modeOptionRemoveRange}
                          </option>
                          <option value="metadata">{modeOptionMetadata}</option>
                          {targetCount === 1 ? (
                            <option value="single">{modeOptionSingle}</option>
                          ) : null}
                        </select>
                      </label>
                      <input
                        ...
                        disabled={pending}
                        onChange={(event) =>
                          onReplaceFromChange(event.target.value)
                        }
                      />
                      <input
                        className="sidebar-rename-seamless-control"
                        ...
                        disabled={pending}
                        onChange={(event) =>
                          onReplaceToChange(event.target.value)
                        }
                      />
                    </div>
                  ) : (
                    <div
                      className="sidebar-rename-mode-row mpx-overlay-merged-top mpx-overlay-seamless-row"
                      aria-label={modeLabel}
                    >
                      ...
                      <div
                        className="sidebar-rename-mode-row-spacer"
                        aria-hidden={
                          mode !== "numbering" && mode !== "remove-range"
                        }
                      >
                        {mode === "numbering" ? (
                          <div className="sidebar-rename-numbering-controls">
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control is-name">
                              <span className="sidebar-rename-numbering-prefix">
                                {numberBaseLabel}
                              </span>
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onNumberBaseChange(event.target.value)
                                }
                              />
                            </label>
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control">
                              ...
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onNumberStartChange(event.target.value)
                                }
                              />
                            </label>
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control">
                              ...
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onNumberStepChange(event.target.value)
                                }
                              />
                            </label>
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control">
                              ...
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onNumberPadWidthChange(event.target.value)
                                }
                              />
                            </label>
                          </div>
                          ...
                          <div
                            className="sidebar-rename-remove-controls"
                            aria-label={`${removeRangeHint} ${removeEdgesHint}`}
                          >
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control">
                              <span className="sidebar-rename-numbering-prefix">
                                {removeHeadPlaceholder}
                              ...
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onRemoveHeadChange(event.target.value)
                                }
                                onKeyDown={(event) => {
                                  handleNumericArrowAdjust(
                                    event,
                                    removeHead,
                                    onRemoveHeadChange,
                                  );
                            ...
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control">
                              ...
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onRemoveTailChange(event.target.value)
                                }
                                onKeyDown={(event) => {
                                  handleNumericArrowAdjust(
                                    event,
                            ...
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control">
                              ...
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onRemoveStartChange(event.target.value)
                                }
                                onKeyDown={(event) => {
                                  const nextStart = handleNumericArrowAdjust(
                                    event,
                                    removeStart,
                                    onRemoveStartChange,
                                  );
                            ...
                            <label className="sidebar-rename-seamless-control sidebar-rename-numbering-control">
                              ...
                              <input
                                ...
                                disabled={pending}
                                onChange={(event) =>
                                  onRemoveEndChange(event.target.value)
                                }
                                onKeyDown={(event) => {
                                  const nextEnd = handleNumericArrowAdjust(
                                    event,
                                    removeEnd,
                                    onRemoveEndChange,
                                  );
                                  ...
                                  const currentStart =
                                    parseNonNegativeInt(removeStart);
                                  if (
                                    nextEnd > 0 &&
                                    currentStart > 0 &&
                  ...
                  <div
                    className="sidebar-rename-preview-table mpx-overlay-merged-bottom"
                  ...
                  >
                    ...
                    >
                      {shouldVirtualizePreviewRows &&
                      previewVirtualRange.topSpacerHeight > 0 ? (
                        <div
                          aria-hidden="true"
                          style={{
                      ...
                      {previewRowsForRender.map((row) => {
                        const failed = Boolean(
                          row.reason && row.reason !== "unchanged",
                        );
                        const unchanged = row.reason === "unchanged";
                        const sourceNameApplied =
                          mode === "replace"
                            ? replaceFrom === row.sourceName
                            : numberBase === row.sourceName;
                        return (
                          <div
                            key={row.nodeId}
                            className={`sidebar-rename-preview-row ${failed ? "is-failed" : ""} ${unchanged ? "is-unchanged" : "is-changed"}`}
                          >
                            {mode === "replace" || mode === "numbering" ? (
                              ...
                              <span
                                className="sidebar-rename-preview-cell"
                            ...
                            <span
                              className="sidebar-rename-preview-arrow"
                              aria-hidden="true"
                            >
                              <MainUiIcon
                                name="next"
                                className="sidebar-rename-preview-arrow-icon"
                              />
                            </span>
                      ...
                      {shouldVirtualizePreviewRows &&
                      previewVirtualRange.bottomSpacerHeight > 0 ? (
                        <div
                          aria-hidden="true"
                          style={{
            ...
            <div className="mpx-overlay-actions mpx-overlay-footer-actions">
              ...
              <button
                className="feature-action-btn main-icon-square-btn sidebar-rename-confirm-btn sidebar-rename-g2-btn mpx-skeuo-metal-btn mpx-overlay-footer-btn"
                type="button"

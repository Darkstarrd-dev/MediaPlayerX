...
function ensureDetailsOpenWithin(
...
function getDetailsBySummaryText(summaryText: string): HTMLDetailsElement {
  ...
  const details = ensuredSummary.closest(
    "details",
...
describe("ThemeParameterPanel", () => {
  ...
  it("常用控件调试页支持变量写入并进入快照", () => {
    ...
    expect(
      screen.getByText("6.2.1 Slider 变体：播放器面板"),
    ).toBeInTheDocument();
    expect(
  ...
  it("大面板层调试按根层、共享总控与分控重排，并切到 fill 三件套", () => {
    ...
    expect(screen.getByText("大面板边框颜色")).toBeInTheDocument();
    expect(screen.getByText("大面板边框宽度")).toBeInTheDocument();
    expect(screen.getByText("大面板背景渐变颜色A")).toBeInTheDocument();
    expect(screen.getByText("大面板背景渐变颜色B")).toBeInTheDocument();
    expect(screen.getByText("大面板背景渐变角度")).toBeInTheDocument();
    ...
    const sharedDetails = getDetailsBySummaryText(
      "3.1 Head / Side / Main 共享总控",
    );
    expect(
      within(sharedDetails).queryByText("--mpx-large-panel-head-fill-start"),
    ).toBeNull();
    expect(screen.getByText("大面板内共用边框颜色")).toBeInTheDocument();
    expect(screen.getByText("大面板内共用边框宽度")).toBeInTheDocument();
    expect(screen.getByText("大面板内共用背景渐变颜色A")).toBeInTheDocument();
    expect(screen.getByText("大面板内共用背景渐变颜色B")).toBeInTheDocument();
    expect(screen.getByText("大面板内共用背景渐变角度")).toBeInTheDocument();
    expectFieldOrder([
    ...
    const settingsDetails =
      getDetailsBySummaryText("3.10.0 设置/帮助/主题参数");
  ...
  it("大容器层调试拆分为三段可折叠，并支持新增槽位调试与快照导出", () => {
    ...
    const workspaceDetails =
      getDetailsBySummaryText("2.3.2.1 工作区 缩略图模式");
    const nameListDetails =
      getDetailsBySummaryText("2.3.2.2 工作区 文件列表模式");
  ...
  it("Header/Sidebar/Main/Metadata 基础外观改为独立折叠，子节层级归位", () => {
    ...
    expect(
      metadataContent?.contains(screen.getByText("2.4.2.4 文件列表")),
    ).toBe(true);
    expectFieldOrderWithin(metadataRootDetails, ["2.4.2.4 文件列表"]);
  });

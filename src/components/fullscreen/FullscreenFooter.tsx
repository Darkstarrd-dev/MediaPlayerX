...
const FULLSCREEN_AUTOPLAY_ICON_NODES: Record<
  FullscreenAutoplayIconName,
  ReactElement
> = {
  autoplayOn: (
    <>
...
const FULLSCREEN_FOOTER_ICON_NODES: Record<
  FullscreenFooterIconName,
  ReactElement
> = {
  swapSides: (
    <>
...
function FullscreenAutoplayIcon({
  name,
}: {
  name: FullscreenAutoplayIconName;
}) {
  return (
...
export function FullscreenFooter({
  ...
  const [openPreviewFormatPopover, setOpenPreviewFormatPopover] =
    useState(false);
  const [openPreviewQualityPopover, setOpenPreviewQualityPopover] =
    useState(false);
  ...
  useEffect(() => {
    setPreviewScaleDraftValue(
      Math.max(0.1, Math.min(1, Number(imageConvertPreviewScale.toFixed(1)))),
    );
  }, [imageConvertPreviewScale]);

  useEffect(() => {
    setPreviewQualityDraftValue(
      Math.max(10, Math.min(100, Math.round(imageConvertPreviewQuality))),
    );
  ...
  const commitPreviewScaleDraft = () => {
    onChangeImageConvertPreviewScale?.(
      Math.max(0.1, Math.min(1, Number(previewScaleDraftValue.toFixed(1)))),
    );
  };

  const commitPreviewQualityDraft = () => {
    onChangeImageConvertPreviewQuality?.(
      Math.max(10, Math.min(100, Math.round(previewQualityDraftValue))),
    );
  };
  ...
  const showPreviewFormatPopover =
    popoverDebugPinned || openPreviewFormatPopover;
  const showPreviewQualityPopover =
    popoverDebugPinned || openPreviewQualityPopover;
  ...
  const autoplayRangePercent = Math.max(
    0,
    Math.min(100, ((autoPlayDraftValue - 1) / 8) * 100),
  );
  const zoomRangePercent = Math.max(
    0,
    Math.min(100, ((zoomDraftValue - 10) / 190) * 100),
  );
  ...
  return (
    ...
    >
      <div
        className={`fullscreen-meta-row ${footerInfoRightText ? "is-dual" : "is-single"}`}
      >
      ...
      <div className="fullscreen-controls-row">
        <div className="fullscreen-group is-left">
          {imageConvertPreviewMode ? null : (
            <>
              <button
                aria-label={
                  fullscreenDisplay === "dual"
                    ? t("ui.fullscreen.singleDisplay")
                    : t("ui.fullscreen.dualDisplay")
                }
                className={`video-action-btn fullscreen-action-btn ${fullscreenDisplay === "dual" ? "is-active" : ""}`}
                type="button"
                data-tooltip-label={
                  fullscreenDisplay === "dual"
                    ? t("ui.fullscreen.singleDisplay")
              ...
              <button
                aria-label={t("ui.fullscreen.swapSides")}
                className="video-action-btn fullscreen-action-btn"
                type="button"
                data-tooltip-label={t("ui.fullscreen.swapSides")}
                disabled={fullscreenDisplay !== "dual"}
              ...
              >
                ...
                >
                  <span className="fullscreen-action-content">
                    <FullscreenAutoplayIcon
                      name={autoPlayEnabled ? "autoplayOn" : "autoplayOff"}
                    />
                  </span>
                </button>
                ...
                >
                  <div
                    ...
                    role="group"
                    aria-label={t("a11y.header.autoPlayLevels")}
                  >
                    <div className="header-vertical-slider-value">
                      {Math.max(1, Math.min(9, Math.round(autoPlayDraftValue)))}
                    </div>
                        ...
                        <SkeuoRunway
                          ...
                          onChange={(event) => {
                            ...
                            const roundedLevel = Math.max(
                              1,
                              Math.min(9, Math.round(nextValue)),
                            );
            ...
            <>
              ...
              >
                <button
                  ...
                  data-tooltip-label={
                    imageConvertPreviewLongestEdgePx != null
                      ? `LongestEdge ${Math.round(imageConvertPreviewLongestEdgePx)} (scale disabled)`
                      : `Scale ${imageConvertPreviewScale.toFixed(1)}`
                  }
                ...
                <div
                  className="header-popover-panel header-popover-panel--upward"
                  data-slot="fs-image-convert-preview-scale-pop"
                  hidden={!showPreviewScalePopover}
                  role="dialog"
                  aria-label="Scale"
                >
                  <div
                  ...
                  >
                    <div className="header-vertical-slider-value">
                      {previewScaleDraftValue.toFixed(1)}
                    </div>
                    <div className="header-vertical-slider-body">
                      <div className="mpx-runway-axis is-vertical">
                        <SkeuoRunway
              ...
              <div
                className={`header-popover-control header-popover-control--upward ${showPreviewFormatPopover ? "is-open" : ""}`}
                onMouseEnter={() => setOpenPreviewFormatPopover(true)}
                onMouseLeave={() => {
                  if (popoverDebugPinned) {
                    return;
                  }
              ...
              >
                <button
                  aria-pressed={showPreviewFormatPopover}
                  className="video-action-btn fullscreen-action-btn header-popover-trigger"
                  type="button"
                  data-tooltip-label={`Format ${imageConvertPreviewFormat.toUpperCase()}`}
                >
                  <span className="fullscreen-action-content">F</span>
                </button>
                <div
                  className="header-popover-panel header-popover-panel--upward fullscreen-convert-format-popover"
                  data-slot="fs-image-convert-preview-format-pop"
                  hidden={!showPreviewFormatPopover}
                  role="dialog"
                  aria-label="Format"
                >
                  <div
                    className="fullscreen-convert-format-options"
                    role="group"
              ...
              >
                <button
                  aria-pressed={showPreviewQualityPopover}
                  className="video-action-btn fullscreen-action-btn header-popover-trigger"
                  type="button"
                  data-tooltip-label={`Quality ${imageConvertPreviewQuality}`}
                >
                  <span className="fullscreen-action-content">Q</span>
                </button>
                <div
                  className="header-popover-panel header-popover-panel--upward"
                  data-slot="fs-image-convert-preview-quality-pop"
                  hidden={!showPreviewQualityPopover}
                  role="dialog"
                  aria-label="Quality"
                >
                  <div
                  ...
                  >
                    <div className="header-vertical-slider-value">
                      {Math.round(previewQualityDraftValue)}
                    </div>
                    <div className="header-vertical-slider-body">
                      <div className="mpx-runway-axis is-vertical">
                        <SkeuoRunway
          ...
          >
            ...
            >
              <span className="fullscreen-action-content">
                {zoomPercent === 100 ? (
                  <FullscreenFooterIcon name="zoomPlus" />
                ) : (
          ...
          <button
            aria-label={t("ui.fullscreen.exit")}
            className="video-action-btn fullscreen-action-btn"
            type="button"
            data-tooltip-label={t("ui.fullscreen.exit")}
            onClick={onExit}
          >
            <span className="fullscreen-action-content">
        ...
        <div className="fullscreen-group is-center">
          {imageConvertPreviewMode ? (
            <>
              <button
                className="video-action-btn fullscreen-action-btn"
                type="button"
                onClick={onConfirmImageConvertPreview}
              >
              ...
              <button
                className="video-action-btn fullscreen-action-btn"
                type="button"
                onClick={onCancelImageConvertPreview}
              >
          ...
          <button
            aria-label={t("ui.fullscreen.prevPage")}
            className="video-action-btn fullscreen-action-btn fullscreen-action-page"
            type="button"
            data-tooltip-label={t("ui.fullscreen.prevPage")}
            onClick={() => onStepFocusedPane(-1)}
          >
            <span className="fullscreen-action-content">
          ...
          <button
            aria-label={t("ui.fullscreen.nextPage")}
            className="video-action-btn fullscreen-action-btn fullscreen-action-page"
            type="button"
            data-tooltip-label={t("ui.fullscreen.nextPage")}
            onClick={() => onStepFocusedPane(1)}
          >
            <span className="fullscreen-action-content">
              <VideoControlIcon name="play" />
          ...
          <button
            aria-label={t("ui.fullscreen.prevPackage")}
            className="video-action-btn fullscreen-action-btn fullscreen-action-page"
            type="button"
            data-tooltip-label={t("ui.fullscreen.prevPackage")}
            disabled={mode !== "image"}
          ...
          <button
            aria-label={t("ui.fullscreen.nextPackage")}
            className="video-action-btn fullscreen-action-btn fullscreen-action-page"
            type="button"
            data-tooltip-label={t("ui.fullscreen.nextPackage")}
            disabled={mode !== "image"}
        ...
        {!hideRightGroup ? (
          <div className="fullscreen-group is-right">
            <button
              aria-label={t("ui.fullscreen.alignUp")}
              className="video-action-btn fullscreen-action-btn"
              type="button"
              data-tooltip-label={t("ui.fullscreen.alignUp")}
              onClick={() => onAlignFocusedPane("up")}
            >
              <span className="fullscreen-action-content">
            ...
            <button
              aria-label={t("ui.fullscreen.alignDown")}
              className="video-action-btn fullscreen-action-btn"
              type="button"
              data-tooltip-label={t("ui.fullscreen.alignDown")}
              onClick={() => onAlignFocusedPane("down")}
            >
              <span className="fullscreen-action-content">
            ...
            <button
              aria-label={t("ui.fullscreen.alignLeft")}
              className="video-action-btn fullscreen-action-btn"
              type="button"
              data-tooltip-label={t("ui.fullscreen.alignLeft")}
              onClick={() => onAlignFocusedPane("left")}
            >
              <span className="fullscreen-action-content">
            ...
            <button
              aria-label={t("ui.fullscreen.alignRight")}
              className="video-action-btn fullscreen-action-btn"
              type="button"
              data-tooltip-label={t("ui.fullscreen.alignRight")}
              onClick={() => onAlignFocusedPane("right")}
            >
              <span className="fullscreen-action-content">

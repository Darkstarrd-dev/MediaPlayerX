...
function renderSection(
  overrides: Partial<ComponentProps<typeof MetadataAdReviewSection>> = {},
) {
...
describe("MetadataAdReviewSection", () => {
  ...
  it("shows focus button state and triggers toggle", () => {
    ...
    const focusButton = screen.getByRole("button", { name: /focus|聚焦/ });
  ...
  it("opens start dialog and passes skip option", () => {
    const { onStartAdReview } = renderSection({
      adReviewTask: createTask("task-pending", "pending"),
    });

    fireEvent.click(screen.getByRole("button", { name: "执行AI广告审核" }));
    expect(
      screen.getByRole("dialog", { name: "选择AI广告审核执行方式" }),
    ).toBeInTheDocument();
  ...
  it("running task pauses immediately without opening start dialog", () => {
    const runningTask = createTask("task-running", "running");
    const { onPauseAdReview } = renderSection({
      adReviewTask: runningTask,
      adReviewQueueTasks: [runningTask],
    });
    ...
    expect(
      screen.queryByRole("dialog", { name: "选择AI广告审核执行方式" }),
    ).toBeNull();
  });
  ...
  it("shows reset-exclusion action and removes hide-unchecked action", () => {
    ...
    expect(
      screen.getByRole("button", { name: /待复核.*1\/1/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "重置剔除" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /隐藏未勾选图片|显示全部图片/ }),
    ).toBeNull();
  });
  ...
  it("hides pending-review content and focus button when review has no candidates", () => {
    ...
    expect(screen.queryByRole("button", { name: /focus|聚焦/ })).toBeNull();
    expect(screen.queryByRole("button", { name: /return|返回/ })).toBeNull();
    expect(screen.queryByRole("button", { name: "重置剔除" })).toBeNull();
    expect(
      screen.getByRole("button", { name: /已完成.*无待复核/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("本轮审核已完成，无待复核内容。"),
    ).toBeInTheDocument();
  });
});

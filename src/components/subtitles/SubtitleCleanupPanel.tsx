...
function SubtitleCleanupPanel({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("a11y.media.subtitleCleanupDialog")}
    ...
    >
      ...
      >
        ...
        <div className="mpx-large-panel-shell settings-shell is-no-side metadata-fetch-shell-wrap">
          <main className="mpx-large-panel-main settings-main metadata-fetch-main mpx-scrollbar-hidden subtitle-cleanup-main">
            <div className="metadata-fetch-shell settings-block mpx-scrollbar-hidden">
              <p className="settings-placeholder">
                {t("ui.media.subtitleCleanupTarget", {
                  label: videoLabel || "-",
                })}
              </p>
              <label>
              ...
              {task?.message ? (
                <p className="settings-placeholder">{task.message}</p>
              ) : null}
              {task?.error_detail ? (
                <p className="settings-danger-text">{task.error_detail}</p>
              ) : null}
              {error ? <p className="settings-danger-text">{error}</p> : null}

              <section
                className="metadata-fetch-preview-card"
                data-slot="fg-main-header-manage-subtitle-cleanup-raw-preview-panel"
              >
                <button
                  type="button"
                ...
                {!rawCollapsed ? (
                  <textarea readOnly value={task?.raw_subtitle_text ?? ""} />
                ) : null}
              </section>

              <section
                className="metadata-fetch-preview-card"
                data-slot="fg-main-header-manage-subtitle-cleanup-clean-preview-panel"
              >
                <button
                  type="button"
                  className="metadata-fetch-preview-toggle"

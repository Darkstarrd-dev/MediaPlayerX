...
describe("buildSettingsPanelProps", () => {
  ...
  it("switches TestStyle to test-skeleton palette pair", () => {
    const updateSettings = vi.fn();
    ...
    props.onStyleChange("TestStyle");

    expect(updateSettings).toHaveBeenCalledWith({
      styleId: "TestStyle",
      paletteDayId: "test-skeleton",
      paletteNightId: "test-skeleton",
      paletteId: "test-skeleton",
      themeId: "test-skeleton",
    });
  });
});

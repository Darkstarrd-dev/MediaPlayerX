...
export function buildManagementPanelProps(
  params: BuildManagementPanelPropsParams,
) {
  const hasSelection =
    params.sidebarSelectedCount > 0 || params.imageSelectedCount > 0;
  const adReviewFeatureVisible =
    params.mode === "image" && params.adReviewFeatureEnabled;

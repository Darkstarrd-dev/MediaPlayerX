...
describe('buildAppHeaderProps', () => {
  ...
  it('存在面板切换 handler 时，非 soft style 也显示面板切换按钮', () => {
    const props = buildAppHeaderProps({
      headerHeight: 56,
      mode: 'image',
      vectorMode: false,
    ...
    expect(props.showPanelToggleControls).toBe(true)
  })

  it('缺少面板切换 handler 时，不显示面板切换按钮', () => {
    const props = buildAppHeaderProps({
      headerHeight: 56,
      mode: 'image',
      vectorMode: false,
    ...
    expect(props.showPanelToggleControls).toBe(false)
  })
})

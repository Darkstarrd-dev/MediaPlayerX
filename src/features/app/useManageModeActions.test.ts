...
function createMoveResponse(
  overrides?: Partial<MoveSidebarNodesResponseDto>,
...
function createParams(
  overrides?: Partial<Parameters<typeof useManageModeActions>[0]>,
) {
  const { backendWrite: _ignoredBackendWrite, ...restOverrides } =
    overrides ?? {};
  const backendWrite = {
  ...
  it("requestManageGroup 打开整理弹窗，confirmManageGroup 后执行分组", async () => {
    const params = createParams({
      backendWrite: {
        ...
        moveSidebarNodes: vi
          .fn()
          .mockResolvedValue(createMoveResponse({ moved_count: 2 })),
      },
    });
    const { result } = renderHook(() => useManageModeActions(params));
    ...
    expect(params.backendWrite.pickDirectoryPath).toHaveBeenCalledWith(
      "选择分组目标目录",
    );
    ...
    expect(params.setManageOperationHint).toHaveBeenCalledWith(
      "分组完成：成功 2 项",
    );

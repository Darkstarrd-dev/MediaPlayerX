...
vi.mock('./useAppReadAndNavigation', () => ({
  useAppReadAndNavigation: (params: unknown) => useAppReadAndNavigationMock(params),
}))

vi.mock('./useAppDisplayAndEffects', () => ({
  useAppDisplayAndEffects: (params: unknown) => useAppDisplayAndEffectsMock(params),
}))
...
describe('useAppDataPipeline integration', () => {
  ...
  it('rerender 时使用最新 runtime 结果，不复用旧依赖', () => {
    ...
    const latestReadCall = useAppReadAndNavigationMock.mock.calls.at(-1)?.[0] as
      | { appSettings?: { mode?: string } }
      | undefined
    const latestDisplayCall = useAppDisplayAndEffectsMock.mock.calls.at(-1)?.[0] as
      | { mediaRepository?: { id?: string } }
      | undefined

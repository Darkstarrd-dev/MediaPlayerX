...
describe('useAudioSidebarState', () => {
  it('音乐侧栏只构建目录节点并标注目录内音频数量', () => {
    ...
    expect(driveNode?.descendantNodeCount).toBe(2)
    expect(driveNode?.directAudioCount).toBe(0)
    expect(driveNode?.descendantAudioFolderCount).toBe(2)
    const audioLeaf = findNodeById(result.current.audioTreeForSidebar, 'audio:X盘/音乐/专辑A/track_a.mp3')

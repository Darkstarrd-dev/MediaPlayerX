...
function createReviewTask(candidateImageIds: string[]): ManageAdReviewTaskDto {
  ...
  return {
    ...
    image_source_by_id: Object.fromEntries(
      candidateImageIds.map((imageId) => [imageId, "llm"] as const),
    ),
    execution: {
    ...
    candidates: candidateImageIds.map((imageId, index) => ({
      image_id: imageId,
      package_id:
        imageId === "img-3" ? "pkg-c" : imageId === "img-2" ? "pkg-b" : "pkg-a",
...
describe("resolveAdReviewPageDerivations", () => {
  ...
  it("treats folder node by kind when imageNodeType is missing", () => {
    const packageById = new Map<string, ImagePackage>([
      [
        "pkg-a",
        createPackage("pkg-a", ["C:", "Users", "A", "pkg-a.zip"], ["img-1"]),
      ],
      [
        "pkg-b",
        createPackage("pkg-b", ["C:", "Users", "A", "pkg-b.zip"], ["img-2"]),
      ],
      [
        "pkg-c",
        createPackage("pkg-c", ["D:", "Other", "pkg-c.zip"], ["img-3"]),
      ],
    ]);
  ...
  it("uses ad-review page index instead of package page index", () => {
    ...
    expect(result.refsInPageBase).toEqual([
      { packageId: "pkg-a", imageIndex: 3 },
    ]);
  });

...
describe("useResolvedMediaState", () => {
  ...
  it("缩略图目标仅解析当前页，避免切换阶段请求风暴", () => {
    ...
    const visibleImageRefs: FocusedImageRef[] = Array.from({ length: 6 }).map(
      (_, index) => ({
      ...
      vi
        ...
        .find((call) =>
          call.targets.some((target) =>
            target.targetId.startsWith("image-thumb:"),
          ),
        ) ?? null;
  ...
  it("缩略图质量和宽度变化会更新缩略图解析参数", () => {
    ...
    const { rerender } = renderHook(
      ({ thumbnailQuality, thumbnailWidth }) =>
        useResolvedMediaState({
          ...
          thumbnailWidth,
          thumbnailGenerationConcurrency: 4,
          thumbnailAdaptiveResolution: false,
          thumbnailQueueSize: 64,
          packageById,
          focusedImage: null,
          metadataImage: null,
    ...
    const firstThumbTarget = firstTargets.find(
      (target) => target.targetId === "image-thumb:img-0",
    );
    ...
    const secondThumbTarget = secondTargets.find(
      (target) => target.targetId === "image-thumb:img-0",
    );
  ...
  it("启用相邻页预热时会在当前页后追加相邻页缩略图目标", () => {
    ...
    const allRefs: FocusedImageRef[] = Array.from({ length: 12 }).map(
      (_, index) => ({
        packageId: packageData.id,
        imageIndex: index,
      }),
  ...
  it("全屏预取深度使用设置值替代硬编码半径", () => {
    ...
    const allRefs: FocusedImageRef[] = Array.from({ length: 20 }).map(
      (_, index) => ({
        packageId: packageData.id,
        imageIndex: index,
      }),
  ...
  it("启用全屏重采样时会追加 fullscreen 目标并携带 kernel", () => {
    ...
    const fullscreenTargets =
      call?.targets.filter((target) =>
        target.targetId.startsWith("image-fullscreen:"),
      ) ?? [];

    expect(fullscreenTargets.map((target) => target.targetId)).toContain(
      "image-fullscreen:img-6",
    );
    expect(fullscreenTargets.map((target) => target.targetId)).toContain(
      "image-fullscreen:img-8",
    );
    expect(fullscreenTargets.map((target) => target.targetId)).toContain(
      "image-fullscreen:img-4",
    );
    expect(
      fullscreenTargets.every(
        (target) => target.fullscreenKernel === "mitchell",
      ),
    ).toBe(true);
  });
  ...
  it("metadataImageSrc 仅使用 original URL，不回退到缩略图", () => {
    const packageData = createPackageWithImages(1);
    const packageById = new Map<string, ImagePackage>([
      [packageData.id, packageData],
    ]);
    type ResolvedUrlsMock = {
    ...
    const { result, rerender } = renderHook(
      ({ resolvedUrls }) => {
        vi.mocked(useResolvedMediaUrls).mockReturnValue(resolvedUrls);
        return useResolvedMediaState({
          repository: {} as MediaRepository,
          benchSettings: createBenchSettings(),
          maxConcurrent: 8,
          actualCellWidth: 240,
          ...
          packageById,
          focusedImage: null,
          metadataImage: packageData.images[0],
          focusedRef: null,
          orderedRootScopedImageRefs: [
            { packageId: packageData.id, imageIndex: 0 },
          ],
    ...
    expect(result.current.metadataImageSrc).toBeNull();
    ...
    expect(result.current.metadataImageSrc).toBe("media://original");
  });
  ...
  it("聚焦音频时会下发 audio 资源解析目标", () => {
    ...
    const firstCall =
      vi
        ...
        .mock.calls.map((call) => call[0])
        .find((call) =>
          call.targets.some((target) => target.targetId.startsWith("audio:")),
        ) ?? null;
    const audioTargetIds = firstCall?.targets
      .filter((target) => target.targetId.startsWith("audio:"))
      .map((target) => target.targetId);
  ...
  it("node browse cover 预热使用独立低并发缩略图队列", () => {
    ...
    const warmupCall =
      vi
        ...
        .find((call) =>
          call.targets.some((target) =>
            target.targetId.startsWith("node-cover-thumb:"),
          ),
        ) ?? null;
  ...
  it("importBusy=true 时缩略图目标数限制为首屏可视区范围", () => {
    ...
    const allRefs: FocusedImageRef[] = Array.from({ length: 30 }).map(
      (_, index) => ({
        packageId: packageData.id,
        imageIndex: index,
      }),
    ...
    const call =
      vi
        ...
        .mock.calls.map((args) => args[0])
        .find((args) =>
          args.targets.some((t) => t.targetId.startsWith("image-thumb:")),
        ) ?? null;

    const thumbTargets =
      call?.targets.filter((t) => t.targetId.startsWith("image-thumb:")) ?? [];
  ...
  it("importBusy=true 时 maxConcurrent 被限制为 2", () => {
    ...
    const allRefs: FocusedImageRef[] = Array.from({ length: 4 }).map(
      (_, index) => ({
        packageId: packageData.id,
        imageIndex: index,
      }),

...
function findNodeByPackageId(
  nodes: ReturnType<typeof buildAdReviewSidebarState>,
  packageId: string,
) {
  const stack = [...nodes];
...
describe("buildAdReviewSidebarState", () => {
  it("aggregates suspected counts to parent folder nodes", () => {
    const packageById = new Map<string, ImagePackage>([
      [
        "pkg-a",
        createPackage("pkg-a", ["C:", "Users", "A", "pkg-a.zip"], "pkg-a"),
      ],
      [
        "pkg-b",
        createPackage("pkg-b", ["C:", "Users", "A", "pkg-b.zip"], "pkg-b"),
      ],
    ]);
    ...
    const compactFolder = drive?.children.find(
      (node) => node.label === "Users/A",
    );

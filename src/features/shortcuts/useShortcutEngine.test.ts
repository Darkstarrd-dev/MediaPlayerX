...
function listenFullscreenRatingFeedback() {
  ...
  const onEvent = (event: Event) => {
    const detail = (event as CustomEvent<FullscreenRatingFeedbackDetail>)
      .detail;
    if (!detail) {
      return;
    }
...
describe("useShortcutEngine ctrl+arrow image mapping", () => {
  ...
  it("maps ctrl+up/down to ctrl-wheel-like sidebar navigation", () => {
    ...
    expect(params.onImageCtrlWheelNavigateSidebar).toHaveBeenNthCalledWith(
      1,
      "prev",
    );
    expect(params.onImageCtrlWheelNavigateSidebar).toHaveBeenNthCalledWith(
  ...
  it("image mode without focused image still allows Enter/F fullscreen shortcuts", () => {
    ...
    expect(params.onSetFullscreenActive).toHaveBeenNthCalledWith(
      1,
      expect.any(Function),
    );
  ...
  it("fullscreen image-only maps Delete to delete-mark handler for image pane", () => {
    const params = createBaseParams();
    params.fullscreenActive = true;
    params.mode = "image";
    params.fullscreenDisplay = "image-only";
    ...
    expect(params.onToggleFullscreenDeleteMark).toHaveBeenCalledTimes(1);
    expect(params.onToggleFullscreenDeleteMark).toHaveBeenCalledWith("image");
  });

  it("fullscreen video-only maps Delete to delete-mark handler for video pane", () => {
    const params = createBaseParams();
    params.fullscreenActive = true;
    params.mode = "video";
    ...
    expect(params.onToggleFullscreenDeleteMark).toHaveBeenCalledWith("video");
  });

  it("fullscreen dual maps Delete to focused pane when video focused", () => {
    const params = createBaseParams();
    params.fullscreenActive = true;
    params.fullscreenDisplay = "dual";
    params.fullscreenVideoFocus = true;
  ...
  it("fullscreen dual maps Delete to focused pane when image focused", () => {
    const params = createBaseParams();
    params.fullscreenActive = true;
    params.fullscreenDisplay = "dual";
  ...
  it("non-fullscreen Delete does not trigger delete-mark handler", () => {
    const params = createBaseParams();
    params.fullscreenActive = false;
    renderHook(() => useShortcutEngine(params));
  ...
  it("fullscreen music mode Delete does not trigger delete-mark handler", () => {
    const params = createBaseParams();
    params.fullscreenActive = true;
    params.mode = "music";
    renderHook(() => useShortcutEngine(params));

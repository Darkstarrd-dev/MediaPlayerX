...
export function toErrorMessage(error: unknown): string {
  return toErrorMessageWithFallback(error, "media_resolve_failed");
}
...
export function isSyncResolveRepository(
  repository: MediaRepository,
): repository is SyncResolveRepository {
  return (
    "resolveMediaResourceSync" in repository &&
    typeof repository.resolveMediaResourceSync === "function"
  );
}
...
export function buildRequestKey(target: MediaResolveTarget): string | null {
  ...
  if (target.variant === "fullscreen") {
    const targetWidth = Math.max(
      1,
      Math.min(7680, Math.round(target.fullscreenTargetWidth ?? 1920)),
    );
    const targetHeight = Math.max(
      1,
      Math.min(4320, Math.round(target.fullscreenTargetHeight ?? 1080)),
    );
  ...
  const maxEdge = Math.max(
    64,
    Math.min(2048, Math.round(target.thumbnailMaxEdge ?? 320)),
  );
  const quality = Math.max(
    THUMBNAIL_MIN_QUALITY,
    Math.min(THUMBNAIL_MAX_QUALITY, Math.round(target.thumbnailQuality ?? 82)),
  );
    ...
    return {
      ...
      thumbnail: {
        ...
        quality: Math.max(
          THUMBNAIL_MIN_QUALITY,
          Math.min(
            THUMBNAIL_MAX_QUALITY,
            Math.round(target.thumbnailQuality ?? 82),
          ),
        ),
        generation_concurrency: Math.max(
          1,
          Math.min(16, Math.round(target.thumbnailGenerationConcurrency ?? 4)),
        ),
        queue_size:
          target.thumbnailQueueSize != null
...
export async function runTasksWithConcurrency(
  ...
  const limit = Number.isFinite(maxConcurrent)
    ? Math.max(1, Math.min(tasks.length, maxConcurrent))
    : tasks.length;
  let cursor = 0;

...
import { mapMediaLocatorDto, mapMediaLocatorToDto } from "../../mediaLocator";
import { deriveWorkTitleFromFileName } from "./utils";

export const toMediaLocatorDto = mapMediaLocatorToDto;

export const toMediaLocatorViewModel = mapMediaLocatorDto;

export function toImageItemDto(
  item: ImagePackage["images"][number],
): ImageItemDto {
  return {
    id: item.id,
...
export function toAudioItemDto(audio: AudioItem): AudioItemDto {
  ...
  return {
    ...
    album: audio.album ?? "",
    author: audio.author ?? "",
    track_title: audio.trackTitle?.trim().length
      ? audio.trackTitle
      : fallbackTrackTitle,

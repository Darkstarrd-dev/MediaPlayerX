...
export function normalizeAdReviewExecution(
  ...
  const maxConcurrency = Number.isFinite(request.max_concurrency)
    ? Math.min(
        ...
        Math.max(
          MIN_AD_REVIEW_MAX_CONCURRENCY,
          Math.floor(request.max_concurrency as number),
        ),
      )
...
export function buildAdReviewAudit(
  ...
  return {
    ...
    llm_hit_rate:
      llmCalls > 0 ? sourceDistribution.llm_suspected / llmCalls : 0,
...
export class MockAdReviewHandlers {
  ...
  private resolveManageAdReviewImageIds(
    request: StartManageAdReviewRequestDto,
  ): string[] {
    ...
    const imageById = new Map<
      string,
      { source: ImagePackageDto; image: ImageItemDto }
    >();
    ...
    const parsedTargets = (request.node_ids ?? [])
      ...
      .filter(
        (
          value,
        ): value is { kind: "folder" | "package" | "video"; pathKey: string } =>
          Boolean(value),
      );
    ...
    for (const source of allSources) {
      ...
      for (const target of parsedTargets) {
        if (
          target.kind === "folder" &&
          pathHasPrefix(sourcePathKey, target.pathKey)
        ) {
          matched = true;
          break;
        }
  ...
  startManageAdReviewSync(
    ...
    for (const imageId of selectedImageIds) {
      ...
      const shouldBeCandidate =
        imageSource === "known-hash" ||
        (imageSource === "llm" && image.ordinal % 2 === 1);
      ...
      candidates.push({
        ...
        hash: hashLocator(`${source.id}:${image.id}`)
          .toString(16)
          .padStart(8, "0"),
      });
    }
  ...
  startManageCoverReviewSync(
    ...
    for (const imageId of selectedImageIds) {
      ...
      const shouldBeCandidate =
        imageSource === "known-hash" ||
        (imageSource === "llm" &&
          (image.ordinal % 3 === 0 || image.ordinal % 2 === 1));
      if (!shouldBeCandidate) {
        continue;
      }
      ...
      candidates.push({
        ...
        reason:
          imageSource === "known-hash"
            ? "mock_known_hash_non_body"
            : "mock_llm_non_body_candidate",
        source: imageSource === "known-hash" ? "known-hash" : "llm",
        hash: hashLocator(`cover:${source.id}:${image.id}`)
          .toString(16)
          .padStart(8, "0"),
      });
    }
    ...
    const task: ManageAdReviewTaskDto = {
      ...
      message:
        candidates.length > 0
          ? `审核完成：疑似非正文 ${candidates.length} 张`
          : "审核完成：未发现疑似非正文",
      error_detail: null,
      candidates,
      created_at_ms: now,
  ...
  testAdReviewVisionModelSync(
    ...
    if (
      endpoint.toLowerCase().includes("fail") ||
      model.toLowerCase().includes("fail")
    ) {
      return testAdReviewVisionModelResponseSchema.parse({
        ok: false,
  ...
  confirmManageAdReviewDeleteSync(
    ...
    deleteImageItemsSync: (
      req: DeleteImageItemsRequestDto,
    ...
    const normalizedIds = Array.from(
      new Set(
        request.image_ids
          ...
          .filter((value) => value.length > 0 && candidateIds.has(value)),
      ),
    );
    ...
    const deletedSet = new Set(
      normalizedIds.filter((imageId) => !failedSet.has(imageId)),
    );
    ...
    const nextTask: ManageAdReviewTaskDto = {
      ...
      candidates: task.candidates.filter(
        (item) => !deletedSet.has(item.image_id),
      ),
      suspected_count: task.candidates.filter(
        (item) => !deletedSet.has(item.image_id),
      ).length,
      scope_image_ids: task.scope_image_ids.filter(
        (imageId) => !deletedSet.has(imageId),
      ),
      image_source_by_id: nextImageSourceById,
      updated_at_ms: now,
      message:
        deletedSet.size > 0
          ? `已删除 ${deletedSet.size} 张疑似广告`
          : task.message,
    };
  ...
  confirmManageCoverReviewHideSync(
    ...
    const appliedCount = Math.min(
      requestedCount,
      Math.max(0, setHiddenResult.updated_count),
    );
    ...
    const nextCandidates = task.candidates.filter(
      (item) => !removedSet.has(item.image_id),
    );
    ...
    const nextTask: ManageAdReviewTaskDto = {
      ...
      scope_image_ids: task.scope_image_ids.filter(
        (imageId) => !removedSet.has(imageId),
      ),
      image_source_by_id: nextImageSourceById,
      updated_at_ms: now,
      message:
        appliedCount > 0 ? `已隐藏 ${appliedCount} 张疑似非正文` : task.message,
    };

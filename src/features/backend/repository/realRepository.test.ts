...
function createImportTaskDto() {
  const now = Date.now();
  return {
    task_id: "task-1",
    task_type: "import" as const,
    source: "dialog-files" as const,
...
function createCoreBackend(
  overrides: Partial<NonNullable<Window["mediaPlayerBackend"]>> = {},
): NonNullable<Window["mediaPlayerBackend"]> {
  const snapshot = createLibrarySnapshotDto();
  const packageItem = snapshot.image_packages[0];
  const imageItem = packageItem.images[0];

  const backend: NonNullable<Window["mediaPlayerBackend"]> = {
    readLibrarySnapshot: async () => snapshot,
    readImageSidebarTree: async () => createSidebarResponseDto(),
    readImagePage: async () => ({
      source_id: packageItem.id,
      total_items: 1,
      page_index: 0,
      page_size: 1,
      refs: [{ package_id: packageItem.id, image_index: 0 }],
    }),
    ...
    writePackageGrade: async (request) => ({
      package_id: request.package_id,
      grade: request.grade,
      updated_at_ms: Date.now(),
    }),
    saveVideoCover: async (request) => ({
...
describe("RealMediaRepository", () => {
  it("IPC 超时会抛出 TimeoutError", async () => {
    window.mediaPlayerBackend = createCoreBackend({
      readLibrarySnapshot: async () =>
        new Promise<LibrarySnapshotDto>(() => undefined),
    });

    const repository = new RealMediaRepository();

    await expect(
      repository.readLibrarySnapshot({ timeoutMs: 25 }),
  ...
  it("支持字幕清理 IPC 调用并解析响应", async () => {
    ...
    const read = await repository.readManageSubtitleCleanupTask({

...
export class MockMediaRepository
  implements MediaRepository, SynchronousMediaRepository
{
  ...
  private readonly subtitleLocalModelsByDir = new Map<
    string,
    ListSubtitleLocalModelsResponseDto["models"]
  >();

  private readonly subtitleDownloadTasks = new Map<
    string,
    SubtitleModelDownloadTaskDto
  >();
  ...
  private readonly subtitleCleanupTasks = new Map<
    string,
  ...
  confirmManageAdReviewDeleteSync(
    ...
    return this.adReview.confirmManageAdReviewDeleteSync(request, (req) =>
      this.deleteImageItemsSync(req),
    );
  }
  ...
  confirmManageCoverReviewHideSync(
    ...
    return this.adReview.confirmManageCoverReviewHideSync(request, (req) =>
      this.setImageHiddenSync(req),
    );
  }
  ...
  async confirmManageCoverReviewHide(
    ...
    return resolveAsync(
      this.confirmManageCoverReviewHideSync(request),
      options,
    );
  }
  ...
  async readManageSubtitleCleanupTask(
    ...
    return resolveAsync(
      this.readManageSubtitleCleanupTaskSync(request),
      options,
    );
  }
  ...
  async writePackageExternalMetadata(
    ...
    return resolveAsync(
      this.writePackageExternalMetadataSync(request),
      options,
    );
  }
  ...
  async startSubtitleModelDownload(
    ...
    const model = this.subtitleRemoteModels.find(
      (item) => item.id === request.model_id,
    );
    if (!model) {
  ...
  async clearSubtitleLocalModel(
    ...
    return resolveAsync(
      { ok: true, removed_path: `${modelDir}/${modelId}`, message: null },
      options,
    );
  }
  ...
  async startSubtitleSession(
    ...
    const fallbackApplied =
      (request.provider_preference === "auto" ||
        request.provider_preference === "directml") &&
      provider === "cpu";
    ...
    return resolveAsync(
      {
        ...
        subtitle_path: this.subtitleSessionState?.running
          ? "mock://subtitle.auto-live.auto.srt"
          : null,
        cue_count: 0,
        updated_at_ms: Date.now(),
      },
  ...
  requestExternalSourceFolderRefreshSync(
  ...
  async requestExternalSourceFolderRefresh(
    ...
    return resolveAsync(
      this.requestExternalSourceFolderRefreshSync(request),
      options,
    );
  }

  setExternalSourceWatcherEnabledSync(
  ...
  async setExternalSourceWatcherEnabled(
    ...
    return resolveAsync(
      this.setExternalSourceWatcherEnabledSync(request),
      options,
    );
  }
}

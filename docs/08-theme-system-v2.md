...
| `--mpx-workspace-fill-start` | `var(--mpx-main-fill-start)` | 工作区填充起点色 |
| `--mpx-workspace-fill-end` | `var(--mpx-main-fill-end)` | 工作区填充终点色 |
| `--mpx-workspace-border-color` | `var(--mpx-main-border-color)` | 工作区边框色 |
...
:root[data-mpx-palette="my-palette"] {
  ...
  /* C. 语义色（必须） */
  --mpx-bg-app: #...;
  --mpx-workspace-fill-start: #...;
  --mpx-workspace-fill-end: #...;
  --mpx-workspace-border-color: #...;
  --mpx-bg-panel: #...;
  --mpx-bg-elevated: #...;
  --mpx-bg-hover: ...;
  --mpx-bg-selected: ...;
...
| 缩略图卡片 | `--mpx-thumbnail-card-{border-color,border-width,bg,focus-bg,selected-bg,shadow,padding,radius}` + 局部覆写 `--mpx-image-grid-card-*` / `--mpx-video-grid-card-*` |

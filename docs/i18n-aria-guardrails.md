...
3. tooltip 字段统一约束
   - renderer 侧 tooltip 统一使用 `data-tooltip-label` 承载，禁止新增 `title` 作为 tooltip 来源。
   - `title` 仅允许用于非 tooltip 语义且需有明确理由（默认不允许）。
...
5. 组件实现约束
   - 组件中禁止新增以下硬编码属性值：`aria-label`、`data-tooltip-label`、`placeholder`、`alt`、tab/button 可见文本。
...
- 不存在新增硬编码 `aria-label/data-tooltip-label/placeholder/alt/按钮文本`。
- `i18n:check` 通过。
- 受影响测试通过，且构建通过。

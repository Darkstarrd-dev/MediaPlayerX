...
目标：在不破坏现有交互/架构门禁的前提下，补齐“AI 模型设置”可用性（命名、持久化、模型测试、路径选择），并修正快捷键展示/捕获与若干交互细节（退出一致性、管理面板布局、缺失源自动清理）。

更新（2026-02-11）：向量模型/Embedding/wd-swin 本地标签模型链路已在后续重构中整体移除，本文件中相关条目仅保留为历史实施记录。

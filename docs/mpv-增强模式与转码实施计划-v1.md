...
- 2026-02-28：已完成 P2 音乐页任务历史与重试首版：
  - 转码面板新增最近任务列表（状态/进度/输出数/消息）与“重试”按钮，重试沿用历史任务请求参数。
  - 新增历史清空操作，避免会话内任务堆积；补充重试缺失参数错误提示文案（i18n）。
  - 新增用例覆盖“按历史任务重试转码”。
- 2026-02-28：已完成 P2 失败任务筛选与一键重试增强：
  - 最近任务区新增“仅失败/显示全部”筛选切换。
  - 新增“一键重试失败项”，会聚合历史失败任务中的音频 ID 并按当前面板参数重新发起任务。
  - 新增失败历史为空与失败任务缺少可重试音频 ID 的错误提示文案（i18n）。
- 2026-02-28：已完成系统设置 MPX_MPV_BIN 目录选择与验证：
  - 在“音频增强模式”新增 `MPX_MPV_BIN` 目录输入框、目录选择按钮与“验证 MPX_MPV_BIN”按钮。
  - 新增主进程 `verifyAudioEngineMpvBin` IPC：校验目录内 `mpv(.exe)`、设置 `process.env.MPX_MPV_BIN`，并回传最新音频引擎状态。
  - 音频引擎控制器支持运行期覆盖 mpv 路径；当目标模式为 mpv 时会按新路径重建引擎实例并刷新状态。

...
4. **模块边界未强化** — 62 处 `ReturnType<typeof>` 隐式契约未替换为显式接口
5. **特性模块封装缺失** — 11 个特性模块中仅 1 个有 barrel export
...
| 零内联文档 | 中 | 高 | **高** | ━ 未改 |
| Hook 签名级联 | 中 | 中 | **中** | ━ 62 处 ReturnType 未改 |
| 媒体类型不可扩展 | 中 | 低 | **低** | ━ 未改 |
...
| 优先级 | 建议 | 投入 | 收益 |
|--------|------|------|------|
| **P1** | 在模块边界定义显式接口，逐步替代 62 处 `ReturnType<typeof>` | 中 | 高 — 代码量已增至 45K 行，隐式契约风险在增长 |

...
| P2-9 major 依赖升级 | ⏸ 部分完成 | 独立分支已完成 `globals@17`、`@types/node@25`、`eslint@10`；`@eslint/js@10` 受上游 peer 限制阻塞，后续不作为评审指标 |
...
- [ ] P2-9 major 依赖升级（独立分支，`@eslint/js` 暂不纳入评审指标）
- [ ] 流程治理：CI 门禁与评估基线固化
...
**执行记录（2026-02-18）**：
- 已按“一个一个升级，失败即回滚”执行：`globals@17`、`@types/node@25`、`eslint@10` 均已通过 `build` + 目标回归测试后提交。
- `@eslint/js@10` 升级被 `eslint-plugin-react-hooks@7.0.1` 的 peer 约束阻塞（仅声明支持到 eslint 9），已回滚失败尝试。
- 结论：`@eslint/js` 后续不作为评审硬指标；待上游生态放开 peer 后再升级。
...
| `npm outdated` | ⚠️ 10 项 | ✅ ≤ 2 项（不含 `@eslint/js`） |
| 综合评级 | B | B+ 或 A |
| 发布就绪 | ❌ No-Go | ✅ Go |

...
css 的触发点
`src/styles/app/sidebar.css:874`（`[data-sidebar-node-role="collapsible-header"]` 根节点基础/active/manage-selected 救星规则，label background → transparent）
`src/styles/app/sidebar.css:1004`（`[data-sidebar-node-role="list-item"]` 媒体节点 active/manage-selected label background → transparent，保留 color/font-weight）
`src/styles/app/manage.css:15`（`.sidebar .sidebar-row.is-manage.is-selected .sidebar-label` background → transparent，保留 box-shadow/transform）
`src/styles/themes/styles/soft-skeuomorphic-components/soft-skeuomorphic.components.part1.css:39`（`.sidebar-tree` 背景消费语义 token）
`src/styles/themes/styles/soft-skeuomorphic-components/soft-skeuomorphic.components.part1.css:679`（`.sidebar-label::before` marker 基础规则，opacity 0）
`src/styles/themes/styles/soft-skeuomorphic-components/soft-skeuomorphic.components.part1.css:766`（`.is-active .sidebar-label.*` box-shadow 规则）
`src/styles/themes/styles/soft-skeuomorphic-components/soft-skeuomorphic.components.part1.css:805`（`.is-manage.is-selected .sidebar-label` box-shadow/transform 规则）
`src/styles/themes/styles/soft-skeuomorphic-components/soft-skeuomorphic.components.part1.css:840`（`.is-active .sidebar-label.*::before` marker 规则）
`src/styles/themes/styles/soft-skeuomorphic-components/soft-skeuomorphic.components.part1.css:900`（`.is-manage.is-selected .sidebar-label.*::before` marker 规则）
...
`src/index.css:1` 声明的层顺序为 `contract, palette-base, palette, theme-skeleton, theme-style, app-base, app-layout, app-component, app-state`。
后声明的层优先级更高，因此 `app-layout`（sidebar.css）和 `app-component`（manage.css）始终覆盖 `theme-style`（part1.css）。

active / manage-selected 的 label background 实际由 app 层控制（设为 `transparent`），theme 层原 6 条 background 规则已作为死代码移除（修复前位于 part1.css:766-915）。
`::before` marker 的 opacity / background 仍由 theme 层（part1.css）控制，marker 是唯一可见的 active / manage-selected 视觉指示。
...
注意：active / manage-selected 的 label background 在 app 层（sidebar.css / manage.css）被 neutralize 为 `transparent`，实际视觉指示仅通过 `::before` marker 呈现。
...
| `containerLayer` | `1.0` 背景层、`2.0 共享壳层`、`2.1 Header`、`2.2 Sidebar`、`2.3 Main`、`2.4 Metadata`、`fg-sidebar-main`、`fg-main-content-image-name-list`、`metadata file list` | 播放器子系统、全屏图像调整、广告审核 overlay | `bg-only`、`bg-plus-container` | 已按共享壳层 + 单容器 frame 结构重排；文件列表派生调试落在 `2.3.2.2` / `2.4.2.4` |
...
| `commonControls` | `6.1` 滚动条基础层 + sidebar-tree 细节覆写、`6.2.0` range 基础层、`6.2.1` runway、`6.2.2` 竖向 slider、`6.2.3` settings slider groove、`6.3` file-list source | 全屏图像调整曲线控件、播放器 HUD 内非 slider 特化控件 | 页内控件样例（scrollbar / slider / file-list） | 当前已补齐 scrollbar detail、settings groove，并新增文件列表源头链路 |

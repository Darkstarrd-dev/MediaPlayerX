...
- 图片：当前主链路已使用真实媒体渲染；仅在资源不可用时回退占位。
- 视频：使用“当前时间/总时长”的数字时钟模拟播放。
- 导入、封面保存等链路已接入 Main 写读通道与任务队列，不再依赖 `console` 模拟输出。
...
- 已完成文件加载前端链路：导入文件/文件夹弹窗、全窗口拖拽、窗口焦点粘贴路径，统一接入后端任务队列。
...
- 已替换世界坐标辅助控件：按 `docs/ref/3dwidgetsolution2.md` 落地全息球体定位控件（独立 Three.js 渲染器、线框球、投影虚线、象限显示），并叠加定制规则：球体与 `XYZ` 轴固定不旋转（`Y-up`、`XZ` 视角对齐控件左右展开）、当前位置圆点以“进入时位置”为相对原点在球内移动并支持越界穿越（从对侧出现）、圆点亮度表达深度远近、保留可指向任意空间角度的前方向箭头；控件尺寸可在设置面板调节。
...
- 已补充后端接入测试：Repository/IPC 覆盖超时、取消、重试、快照回退；UI 主测仍可能输出非阻断性 act 警告。

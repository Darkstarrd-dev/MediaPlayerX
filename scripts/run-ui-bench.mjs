...
const config = {
  ...
  resolvedMedia: {
    ...
    maxConcurrent: args['max-concurrent'] ? Number(args['max-concurrent']) : undefined,
  },
  imageLoadingSkeleton: {
    mode: args['skeleton'] ?? undefined,
  },
  e2e: {
    importPaths: args['import-path'] ? [String(args['import-path'])] : undefined,
    browseSteps: args['browse-steps'] ? Number(args['browse-steps']) : undefined,
  ...
  dom: {
    ...
    resolveDelayMinMs: args['resolve-delay-min-ms'] ? Number(args['resolve-delay-min-ms']) : undefined,
    resolveDelayMaxMs: args['resolve-delay-max-ms'] ? Number(args['resolve-delay-max-ms']) : undefined,
  },
}
